...
import (
	...
	overlayMock "github.com/strabox/caravela-sim/overlay"
...
func NewChordMock(numNodes int) *Mock {
	chordMock := &Mock{
		numNodes:        numNodes,
...
func (chord *Mock) Init() {
	...
	fmt.Println("CHORD UNSORTED NODES")
	chord.Print()
	sort.Sort(chord)
	fmt.Println("CHORD SORTED NODES")
	chord.Print()
...
func (chord *Mock) Lookup(key []byte) ([]*overlay.Node, error) {
	...
	for index, test := range res {
		if test == nil {
			fmt.Printf("MERDA no indice: %d\n", index)
		}
	}
	return res, nil
}
